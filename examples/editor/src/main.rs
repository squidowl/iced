...
impl Highlighter for ConfigHighlighter {
    ...
    fn highlight_line(&mut self, line: &str) -> Self::Iterator<'_> {
        ...
        let syntax_spans = self.inner.highlight_line(line);

        let Some(errors) = self.error_map.get(&current_line) else {
            return Box::new(
                syntax_spans.map(|(range, highlight)| (range, Highlight::Syntax(highlight))),
            );
        };
        ...
        for (range, syntax) in syntax_spans {
            ...
            while cursor < range.end {
                ...
                let (end, highlight) = match errors.get(err_idx) {
                    Some(err) if err.start <= cursor => {
                        (err.end.min(range.end), Highlight::Error(syntax))
                    }
                    Some(err) => (err.start.min(range.end), Highlight::Syntax(syntax)),
                    None => (range.end, Highlight::Syntax(syntax)),
                };
